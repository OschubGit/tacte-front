...
login\:%:
	...
	if [ $$CONTAINER_EXISTS -eq 1 ]; then \
		...
		docker exec -it $$CONTAINER sh -c 'if command -v bash &> /dev/null; then bash; else sh; fi'; \
	else \
		tput setaf 1; /bin/echo "Service $* not exists"; tput sgr0; \
	fi;
...
bash\:%:
	...
	@docker compose -p ${COMPOSE_PROJECT} --env-file ./.env.docker -f ./docker/${TEMPLATE}/docker-compose.dev.yml run --rm --build --tty --entrypoint "" $* sh -c 'if command -v bash &> /dev/null; then bash; else sh; fi';
	@exit 0;
